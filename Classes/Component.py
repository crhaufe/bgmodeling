...
class Component:
	...
	def GetCosmogenicDecayFactor(self):
		...
		timeconstDict = {
			'Th': 1.405e10 * 365.25 * 1.44,
			'U': 4.468e9 * 365.25 * 1.44,
			'Pb': 22.3 * 365.25 * 1.44,
			'Co': 5.2713 * 365.25 * 1.44,
			'57Co': 271.74 * 1.44, #Table value already in days
			'68Ge': 270.95 * 1.44, #Table value already in days
			'K': 1.248e9 * 365.25 * 1.44,
			'54Mn': 312.03 * 1.44, #Table value already in days
			}
		config = self.component_dict['config']
		dC = self.component_dict['decay_chain']
		if dC == '0v' or dC == '2v' or dC == 'Rn' or dC == 'Th228' or dC == 'DCR' or dC == 'PbBrem':
			...
			if 'DS1-2' in config: #average activity for the DS1-2 multidataset
			...
			elif 'DS3-6' in config: #average activity for theh DS3-6c multidataset
				t_offset = 422.27 #days, from run 2580 (beg DS0) to run 16797 (beg DS3)
				b = 1173 #days, from run 16797 (beg DS3) to run 57103 (end DS6c)
				tau = timeconstDict[dC]
				j = b/tau
				k = -1*j
				t_mean = (tau*(1-(j+1)*mt.exp(k)))/(1-mt.exp(k))
				t_eff = t_offset + t_mean
				cosmo_decay_factor = mt.exp((-1*t_eff)/tau)
			elif 'DS7' in config: #average activity for the DS7 mulitdataset
				t_offset = 1611.09 #days, from run 2580 (beg DS0) to run 57623 (beg DS7)
				b = 275.04 #days, from run 57623 (beg DS7) to run 66813 (end DS7)
				tau = timeconstDict[dC]
				j = b/tau
				k = -1*j
				t_mean = (tau*(1-(j+1)*mt.exp(k)))/(1-mt.exp(k))
