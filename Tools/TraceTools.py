...
class TraceTools:
    ...
    def CalcPointEstimates(self):
        ...
        for var in model.pymc_model.deterministics: # for i, varname in enumerate(trace.varnames)
            ...
            print('    %s %.2E' % (varname, d[varname]['mode']) )
    ...
    def ImportanceSampling(self, nsamples):
        ...
        for submodel in model.submodel_list:
          ...
          samples[submodel] = truncnorm.rvs(0., np.inf, loc = means[submodel], scale = stds[submodel], size = nsamples)
    ...
    #def get_mu(self, activities):
    #    w = tt.stack(activities)
    #    logp = tt.log(w) + tt.log(pdf_effs).T
    #    return tt.sum(tt.exp(logp), axis=1)
    
    #def get_w(self, activities):
    #    return tt.stack(activities)

    #def w_compiled(self):
    #    activities = tt.vector()
    #    result = get_w(activities)
    #    return theano.function(activties,)

    #def marg_likelihood_calc(self, data, sample):
    #    """
    #    Function to actually calculate the marginal likelihood.
    #    """
    #    return sum(likelihood.logp(data)).eval() + prior.logp(sample).eval() - approx_prior.logp(sample).eval()
        #return data * sample

    #def marg_likelihood_calc_compiled(self):
    #    """
    #    Compile theano function to calculate marginal likelihood.  We only want to compile this once.
    #    """
    #    data = tt.vector()
     #   sample = tt.scalar()
     #   activities = tt.vector()
     #   mu = self.get_mu(activities)
     #   likelihood = pm.Poisson.dist(mu = mu)
     #   result = self.marg_likelihood_calc(data, sample)
      #  return theano.function([data,sample], result)
    ...
    def CalculateMarginalLikelihood_v2(self, samples):
        ...
        for submodel in model.submodel_list:
          ...
          approx_prior = truncnorm(0., np.inf, loc = mean, scale = std)
          ...
          for sample in tqdm(samples[submodel], desc = 'Calculating marginal likelihoods for submodel ' + submodel.name + '...'):
            if mt.isinf(prior.logpdf(sample)):
              skip_value += 1
              continue
            p_mean_vals[float_index] = sample
          ...
          #print(p)
          marg_likelihood[submodel.name] = sum(p)/(nsamples-skip_value)
          saved_skip_value += skip_value
        ...
        return marg_likelihood, saved_skip_value
